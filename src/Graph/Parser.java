...
public class Parser {
	...
	{
		try {
			final Function<Node, Integer> key = node -> node.getId();
	...
	{
		try{
			final Function<Road, Integer> key = road -> road.getRoadId();
	...
	public static Collection<Edge> parseSegments(
			File segments,
			Graph graph)
	{
		try{
		    final Function<String[], Edge> toSegment =
                    tokens -> {
                        int roadID = asInt(tokens[0]);
						...
						for (int i = 4; i < tokens.length; i++) {
							coords.add(Double.parseDouble(tokens[i]));
						}
						Edge edge = new Edge(
								roadID,
								length, graph.getNodeOfId(node1ID),
								graph.getNodeOfId(node2ID), coords
						);
